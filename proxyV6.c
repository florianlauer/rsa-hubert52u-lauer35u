...
#define SOMEADDR {{{0x00,0x00, 0x0d,0x00, 0x00,0x00, 0x00,0x00, \
  ...
  if(pid==0) {
    ...
    */
    ...
    if(((strncmp(t1,"GET",3)==0))&&((strncmp(t3,"HTTP/1.1",8)==0)||(strncmp(t3,"HTTP/1.0",8)==0))&&(strncmp(t2,"http://",7)==0))
    {
      ...
      host_addr.sin6_family=AF_INET6;     // IPV6

      bcopy((char*)host->h_addr,(char*)&host_addr.sin6_addr.s6_addr,host->h_length);

      sockfd1=socket(AF_INET6,SOCK_STREAM,IPPROTO_TCP);
      newsockfd1=connect(sockfd1,(struct sockaddr*)&host_addr,sizeof(struct sockaddr));
      sprintf(buffer,"\nConnected to %s  IP - %s\n",t2,inet_ntop(AF_INET6, &host_addr.sin6_addr, straddr,
          sizeof(straddr)));
      if(newsockfd1<0)
